...
const Score = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `${$$result.title = `<title>Score - FCoder</title>`, ""}`, ""}

<div class="${"container svelte-177lsr"}"><canvas id="${"canvas"}" width="${"300"}" height="${"209"}" class="${"svelte-177lsr"}">Sorry, you browser does not support canvas.
  </canvas>

  <h3 class="${"svelte-177lsr"}"><span>Correct:</span>
    <span>${escape($tpCorrectWords)} ${escape($tpCorrectWords > 1 ? "words" : "word")}</span></h3>
  <h3 class="${"svelte-177lsr"}"><span>Wrong:</span>
    <span>${escape($tpWrongWords)} ${escape($tpWrongWords > 1 ? "words" : "word")}</span></h3>
  <h3 class="${"svelte-177lsr"}"><span>Time:</span>
    <span>${escape($tpSeconds)} secs</span></h3>
  ${`<p class="${"svelte-177lsr"}"><code class="${"placeholder svelte-177lsr"}"></code></p>`}
</div>`;
});
export {
  Score as default
